...
public class Patient extends JFrame implements ActionListener, MouseListener,IPatient {
	...
	public Patient (SecondPage sp)
	{
		...
		patLabel.setBounds(30, 30, 200, 50);
		patLabel.setForeground(Color.BLUE);
		patLabel.setFont(myFont1);
        panel.add(patLabel);
	...
	{    
		for(int i=0;i<account.length;i++)
		{
		  if (account[i]!=null)
		   { 
			JOptionPane.showMessageDialog(null,i+1+" .Patient Name: "+account[i].getName());
